  ...
  try {
    ...
    for (const user of usersWithMemberships) {
      ...
      for (const userMembership of user.userMemberships) {
        ...
        // Check groups
        const userGroups = user.groupMembers.map(gm => gm.group);
        const userGroupIds = userGroups.map(g => g.id);
        
        for (const requiredGroup of requiredGroups) {
          if (!userGroupIds.includes(requiredGroup.id)) {
            userErrors.push(`Missing GROUP: ${requiredGroup.title} (${requiredGroup.id})`);
          }
        }
        
        // Check courses
        const userCourses = user.courseEnrollments.map(ce => ce.course);
        ...
        for (const userGroup of userGroups) {
          if (allRequiredGroupIds.includes(userGroup.id)) {
            ...
            if (!hasAccess) {
              userErrors.push(`EXCESS GROUP: ${userGroup.title} (${userGroup.id}) - not allowed for ${membership.title}`);
            }
          }
        }
      ...
      if (userErrors.length > 0) {
        ...
        console.log(`   Groups: ${user.groupMembers.length}, Courses: ${user.courseEnrollments.length}`);
