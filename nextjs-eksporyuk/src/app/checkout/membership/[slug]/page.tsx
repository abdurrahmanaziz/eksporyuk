  ...
  useEffect(() => {
    const fetchMembership = async () => {
      try {
        ...
        const response = await fetch(`/api/memberships/packages?includeInactive=false`)
        ...
        // API returns { success: true, packages: [...] } format
        ...
        // Find by slug OR checkoutSlug
        const foundMembership = data.find((m: Membership) => 
          (m.slug === slug || m.checkoutSlug === slug) && m.isActive !== false
        )
    ...
    const loadUserData = async () => {
      if (session?.user) {
        ...
        try {
          ...
          console.error('[Checkout Membership] Error fetching profile:', error)
        }
      }
    }
