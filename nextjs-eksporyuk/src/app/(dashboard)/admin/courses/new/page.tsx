...
export default function NewCoursePage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Fetch mentors - optional, if API doesn't exist just skip
      try {
      ...
      // Fetch groups - optional, if API doesn't exist just skip
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        affiliateCommissionRate: parseFloat(formData.affiliateCommissionRate) || 30
      }
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="affiliateCommissionRate">
                  {formData.commissionType === 'PERCENTAGE' ? 'Komisi Affiliate (%)' : 'Komisi Affiliate (Rp)'}
                </Label>
                <Input
                ...
                />
                <p className="text-sm text-muted-foreground">
                  Komisi yang diberikan ke affiliate per transaksi
                </p>
              </div>
            </div>
          </CardContent>
