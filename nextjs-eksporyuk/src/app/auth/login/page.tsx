...
function LoginForm() {
  ...
  const handleFacebookLogin = async () => {
    try {
      ...
      console.log('[LOGIN] Facebook signIn result:', result)
      
      if (!result) {
        console.error('[LOGIN] Facebook signIn returned undefined/null')
        setError('Gagal menginisiasi login Facebook. Provider mungkin tidak aktif.')
      }
    } catch (error) {
      console.error('[LOGIN] Facebook login error:', error)
      setError('Gagal login dengan Facebook: ' + (error instanceof Error ? error.message : String(error)))
    }
  }
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full h-12 text-base bg-[#1877F2] hover:bg-[#166FE5] text-white border-[#1877F2] hover:border-[#166FE5]"
