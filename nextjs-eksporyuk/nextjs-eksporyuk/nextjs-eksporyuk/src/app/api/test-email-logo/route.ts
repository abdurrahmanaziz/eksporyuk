...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[TEST_EMAIL] Logo URL:', logoUrl)
    console.log('[TEST_EMAIL] Company:', settings?.emailFooterCompany)
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f3f4f6; padding: 40px 0;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <p style="margin: 16px 0; line-height: 1.6; color: #374151; font-size: 14px; color: #6b7280;">
    ...
    // Send via Mailketing
    console.log('[TEST_EMAIL] Sending to:', to)
    ...
    if (result.success) {
      return NextResponse.json({
        ...
        logoUrl,
        note: 'Check your inbox (and spam folder) in a few moments. Logo should be visible in the email header!'
      })
    } else {
      return NextResponse.json(
        { 
