...
async function fixMembershipDurations() {
  ...
  try {
    ...
    for (const tx of transactions) {
      ...
      if (!sejoliOrderId) {
        console.log(`⚠️  Transaction ${tx.id} has no sejoliOrderId in metadata`);
        stats.noOrder++;
        continue;
      }
      ...
      console.log(`✅ User ${tx.userId}: ${durationType} (${status}) - ${monthsRemaining} ${durationType === 'LIFETIME' ? '' : 'months'} remaining`);
    }
    ...
    console.log(`├─ No order ID: ${stats.noOrder}`);
