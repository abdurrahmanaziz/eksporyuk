...
export default function EditSupplierPackagePage() {
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      try {
        const res = await fetch(`/api/admin/supplier/packages/${id}`)
        if (!res.ok) throw new Error('Failed to fetch package')

        const data = await res.json()
        const pkg = data.success ? data.data : data
        ...
        setForm({
          ...
          affiliateCommissionRate: pkg.affiliateCommissionRate?.toString() || '30'
        })
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Error loading package')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleFeatureChange = (feature: keyof typeof form.features, value: any) => {
    setForm(prev => ({
      ...prev,
      features: {
        ...prev.features,
  ...
  if (loading) {
    return (
      <div className="p-6">
        <div className="text-center text-gray-500">Memuat data paket...</div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Paket Supplier</h1>
