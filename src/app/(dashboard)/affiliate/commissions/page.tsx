...
import { Coins, Clock, CheckCircle2, XCircle, TrendingUp, RefreshCw, ChevronLeft, ChevronRight, Loader2, Receipt } from 'lucide-react'
...
export default function AffiliateCommissionsPage() {
  ...
  const [stats, setStats] = useState<Stats>({ totalCommissions: 0, pendingAmount: 0, paidAmount: 0, refundedAmount: 0, totalTransactions: 0, pendingCount: 0, paidCount: 0 })
  ...
  const fetchCommissions = async () => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ status: statusFilter, period: periodFilter, page: page.toString(), limit: '20' })
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Lihat riwayat komisi dari transaksi affiliate Anda</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Komisi</CardTitle><Coins className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(stats.totalCommissions)}</div><p className="text-xs text-muted-foreground">{stats.totalTransactions} transaksi</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Pending</CardTitle><Clock className="h-4 w-4 text-yellow-500" /></CardHeader><CardContent><div className="text-2xl font-bold text-yellow-600">{formatCurrency(stats.pendingAmount)}</div><p className="text-xs text-muted-foreground">{stats.pendingCount} menunggu</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Sudah Dibayar</CardTitle><CheckCircle2 className="h-4 w-4 text-green-500" /></CardHeader><CardContent><div className="text-2xl font-bold text-green-600">{formatCurrency(stats.paidAmount)}</div><p className="text-xs text-muted-foreground">{stats.paidCount} dibayar</p></CardContent></Card>
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Receipt className="h-5 w-5" /> Riwayat Komisi</CardTitle></CardHeader>
        <CardContent>
                ...
                <TableBody>
                  {commissions.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell><div className="font-medium">{c.transaction?.buyer?.name || '-'}</div><div className="text-xs text-muted-foreground">{c.transaction?.buyer?.email}</div></TableCell>
                      ...
                      <TableCell className="text-right">{formatCurrency(c.transaction?.amount || 0)}</TableCell>
                      ...
                      <TableCell className="text-center">{getStatusBadge(c)}{c.paidAt && <div className="text-xs text-muted-foreground mt-1">{formatDate(c.paidAt)}</div>}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
