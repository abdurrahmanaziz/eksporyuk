...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status === 'pending') statusFilter.paidOut = false
    else if (status === 'paid') statusFilter.paidOut = true
    else if (status === 'refunded') statusFilter.status = 'REFUNDED'

    const whereClause = { affiliateId: affiliateProfile.id, ...dateFilter, ...statusFilter }
    ...
    const allConversions = await prisma.affiliateConversion.findMany({
      where: { affiliateId: affiliateProfile.id, ...dateFilter },
